...
function draw(ctx) {
  ...
  // Calculate points to make triangles
  const cellSizeLimit = variation * imageScale * cellSize * imageScale;
  for (var x = -border * imageScale;
       x < ctx.canvas.width + border * imageScale + cellSize * imageScale;
       x += cellSize * imageScale) {
    for (var y = -border * imageScale;
         y < ctx.canvas.height + border * imageScale + cellSize * imageScale;
         y += cellSize * imageScale) {
      points.push([
        x + getRandomInt(-cellSizeLimit, cellSizeLimit),
        y + getRandomInt(-cellSizeLimit, cellSizeLimit)
      ]);
    }
  }
  drawImage(ctx);
...
function drawImage(ctx) {
  ...
  var convertedColors = [];
  for (var i = 0; i < paletteSize; i++) convertedColors.push(chroma.hsl(colors[i][0], colors[i][1], colors[i][2]));
